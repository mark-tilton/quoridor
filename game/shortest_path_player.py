            ...
            if r.randint(0, 1) == 0 and self.wall_count > 0:
                ...
                for point in points:
                    if (board_state.is_wall_index_in_bounds(point) and board_state.walls[point.x, point.y] == 0):
                        action = create_block(point, orientation)
                        break
            else:
                ...
                action = self.get_best_move(self.pos, board_state, distance_matrix)

            move = Move(board_state, self.index, action)
            if validation.validate_move(self, opponent, move):
                return action
